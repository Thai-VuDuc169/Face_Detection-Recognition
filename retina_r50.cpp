...
int main(int argc, char** argv) 
{
    ...
    std::string DATASET_PATH ("/home/thaivu/Projects/Face_Detection-Recognition/WIDERFACE/Splited_Images/");
    std::string PATH_WRITED_FOLDER ("/home/thaivu/Projects/Face_Detection-Recognition/TRT_Retinaface/Benchmark_Result/");
    std::vector<std::string> files;
    read_files_in_dir(DATASET_PATH.c_str(), files);
    // std::cout << "files.size() = " << files.size() << std::endl;
    // std::cout << "files[9] : " << files[9] << std::endl;     

    std::string temp_path;
    std::string temp_write_pathfile;
    for (auto file : files) 
    {
        
        // std::cout << "The current file namefile: " << file << std::endl;
        temp_path = DATASET_PATH + file;
        // std::cout << temp_path << std::endl;
        cv::Mat img = cv::imread(temp_path.c_str());
        ...
        for (int b = 0; b < BATCH_SIZE; b++)
        {
            ...
            temp_write_pathfile = PATH_WRITED_FOLDER + file.replace(file.end()-4, file.end(), ".txt");
            // std::cout << "temp_write_pathfile: " << temp_write_pathfile << std::endl;
            write_files_in_dir(img, temp_write_pathfile, res, INPUT_W, INPUT_H);
        }
    }
    ...
    // free(dir);      // free() cú pháp của C
    // free(diread);   // delete cú pháp của C++, cho phép gọi deconstructor
    // return EXIT_SUCCESS;
