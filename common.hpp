...
static inline void write_files_in_dir(cv::Mat &img, std::string &file_name, 
                                        const std::vector<decodeplugin::Detection> &res, 
                                        int input_w, int input_h)
{
    ...
    for (size_t j = 0; j < res.size(); j++)
    {
        ...
        if (r_h > r_w) 
        {
            l = res[j].bbox[0] / r_w;
            r = res[j].bbox[2] / r_w;
            t = (res[j].bbox[1] - (input_h - r_w * img.rows) / 2) / r_w;
            b = (res[j].bbox[3] - (input_h - r_w * img.rows) / 2) / r_w;
        } 
        else 
            {
                l = (res[j].bbox[0] - (input_w - r_h * img.cols) / 2) / r_h;
                r = (res[j].bbox[2] - (input_w - r_h * img.cols) / 2) / r_h;
                t = res[j].bbox[1] / r_h;
                b = res[j].bbox[3] / r_h;
            }
        fileout << "face" << " " << res[j].class_confidence << " " << l << " " << t << " " << r-l << " " << b-t << "\n";
    }
    // write to a specific file with the name
    fileout.close();
}
